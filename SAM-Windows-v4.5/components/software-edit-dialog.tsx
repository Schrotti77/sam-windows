...
export function EditSoftwareDialog({ 
  ...
  useEffect(() => {
    if (software) {
      // Initialize form data with current software values
      setFormData({
        ...software,
        vendorId: software.vendor.id,
        // Format dates for HTML date inputs
        releaseDate: software.releaseDate ? new Date(software.releaseDate).toISOString().split('T')[0] : '',
        installationDate: software.installationDate ? new Date(software.installationDate).toISOString().split('T')[0] : '',
        lastPatchDate: software.lastPatchDate ? new Date(software.lastPatchDate).toISOString().split('T')[0] : '',
        supportExpiry: software.supportExpiry ? new Date(software.supportExpiry).toISOString().split('T')[0] : '',
        plannedDecommission: software.plannedDecommission ? new Date(software.plannedDecommission).toISOString().split('T')[0] : '',
      })
    }
  }, [software, vendors])
  ...
  const handleSubmit = async () => {
    if (!software || !formData.name || !formData.category || !formData.vendorId) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>
            Edit details for "{software.name}"
          </DialogDescription>
        </DialogHeader>
